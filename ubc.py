...
from message import MsgKey,RcvMsgIdValue,RspMsgIdValue
...
class Ubc(object):
    ...
    def func_timer(cls):
        if cls.report_imsi_num_4G_tmp is 0:#同步一下
            ...
            if (cls.report_imsi_num_4G_tmp + 10) <  cls.report_imsi_num_4G:
                #todo send tac
                cls._response(('', 6070), id=RcvMsgIdValue.ubc_chg_tac)
                cls.report_imsi_num_4G_tmp = cls.report_imsi_num_4G
    ...
    dispatch[RcvMsgIdValue.ntc_heartbeat] = _process_ntc_heartbeat
    ...
    dispatch[RcvMsgIdValue.ntc_query_imsi] = _process_ntc_query_imsi
    ...
    def _process_ntc_insert_imsi(self, message, address):
        ...
        for imsi in imsi_list:
            table_name, store_code = Tool.parse_imsi(imsi, mnc_2_tablename)
            row = self.database.query(table_name, store_code)
            if not row:
                try:
    ...
    dispatch[RcvMsgIdValue.ntc_insert_imsi] = _process_ntc_insert_imsi
    ...
    def _process_ubc_get_imsi(self, message, address):
        ...
        if table_name == mobile_table_name and self.mobile_4G_empty_ue_num <= 0 or \
            ...
            response(address, id=RspMsgIdValue.ubc_set_imsi,
        ...
        try:
            ...
            # 该imsi没翻译过，指派到2G
                ...
                self.database.insert(table_name, store_code)
                # 将指派的imsi发送给2g
                response(self.address_2GC, id=RspMsgIdValue.ntc_set_imsi,
                         imsilist=[imsi], modulelist=[table_name])
                self._ntc_update_empty_ue_num(table_name)
        except Exception as e:
            logger.error(str(e.args) + str(e.message))
    dispatch[RcvMsgIdValue.ubc_get_imsi] = _process_ubc_get_imsi
    ...
    dispatch[RcvMsgIdValue.ubc_heartbeat] = _process_ubc_heartbeat
    ...
    def _ntc_update_empty_ue_num(self, table_name):
        ...
        if table_name == mobile_table_name:
            if self.mobile_4G_empty_ue_num > 0:
                self.mobile_4G_empty_ue_num -= 1
        elif table_name == union_table_name:
            if self.union_4G_empty_ue_num > 0:
    ...
    def _process_message(self, message, address):
        ...
        """
        logger.debug('message:' + message + 'address:' + str(address))
        ...
        if message:
            try:
                ...
                if id is None:
                    logger.error('unrecognized message from:' + str(address) + ' message:' + str(message))
                    return False
                _func = dispatch.get(id)
                if _func is None:
                    logger.error('id:' + str(id) + ' can not find func')
